...
namespace Ow.Game.Objects
{
    class Player : Character
    {
        ...
        public void CheckRank(Player player)
        {
            ...
            if(this.Data.experience > 0 && this.Data.honor > 0)
            {    
                List<Player> players = new List<Player>(); 

                using (var mySqlClient = SqlDatabaseManager.GetClient())
                {
                    var reader = mySqlClient.ExecuteQueryTable($"SELECT * FROM player_accounts WHERE rankId != 21");
                    
                   foreach(DataRow row in reader.Rows)
                    {
                        players.Add(QueryManager.GetPlayer(int.Parse(row.ItemArray[0].ToString())));
                ...
                {
                    long rankValue = (long)(pl.Data.experience * 0.0001 + pl.Data.honor * 0.01);
                    pl.RankPoints = (int)rankValue;
                }

                var sortedPlayers = players.OrderByDescending(x => x.RankPoints).ToList();
                for (int i = 0; i < sortedPlayers.Count; i++)
                {
                    sortedPlayers[i].Rank = i+1;
                    if (i <= 0)
                    {
                        sortedPlayers[i].RankId = 20;
                    }
                    else if (i <= 2)
                    {
                        sortedPlayers[i].RankId = 19;
                    }
                    else if (i <= 5)
                    {
                        sortedPlayers[i].RankId = 18;
                    }
                    else if (i <= 9)
                    {
                        sortedPlayers[i].RankId = 17;
                    }
                    else if (i <= 14)
                    {
                        sortedPlayers[i].RankId = 16;
                    }
                    else if (i <= 20)
                    {
                        sortedPlayers[i].RankId = 15;
                    }
                    else if (i <= 27)
                    {
                        sortedPlayers[i].RankId = 14;
                    }
                    else if (i <= 35)
                    {
                        sortedPlayers[i].RankId = 13;
                    }
                    else if (i <= 45)
                    {
                        sortedPlayers[i].RankId = 12;
                    }
                    else if (i <= 55)
                    {
                        sortedPlayers[i].RankId = 11;
                    }
                    else if (i <= 66)
                    {
                        sortedPlayers[i].RankId = 10;
                    }
                    else if (i <= 78)
                    {
                        sortedPlayers[i].RankId = 9;
                    }
                    else if (i <= 90)
                    {
                        sortedPlayers[i].RankId = 8;
                    }
                    else if (i <= 102)
                    {
                        sortedPlayers[i].RankId = 7;
                    }
                    else if (i <= 150)
                    {
                        sortedPlayers[i].RankId = 6;
                    }
                    else if (i <= 250)
                    {
                        sortedPlayers[i].RankId = 5;
                    }
                    QueryManager.SavePlayer.Rank(sortedPlayers[i]);
                }
            }
