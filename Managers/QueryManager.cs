...
namespace Ow.Managers
{
    class QueryManager
    {
        public class SavePlayer
        {
            ...
            {
                using (var mySqlClient = SqlDatabaseManager.GetClient())
                    mySqlClient.ExecuteNonQuery($"UPDATE player_accounts SET rankId = {player.RankId}, rankPoints = {player.RankPoints}, rank = {player.Rank} WHERE userId = {player.Id}");
            ...
            try
            {
                using (var mySqlClient = SqlDatabaseManager.GetClient())
                {
                    ...
                    foreach (DataRow row in data.Rows)
                    {
                        ...
                        var rankPoints = Convert.ToInt32(row["rankPoints"]);
        ...
        public static void SetEquipment(Player player)
        {
            try
            {
                ...
                using (var mySqlClient = SqlDatabaseManager.GetClient())
                {
                    ...
                    var booty = mySqlClient.ExecuteQueryTable($"SELECT * FROM player_accounts WHERE userId = {player.Id}");
                    var itemsBase = new ItemsBase(0);
                    foreach (DataRow row in booty.Rows)
                    {
                        dynamic bootyKeys = JsonConvert.DeserializeObject(row["bootyKeys"].ToString());
                        var temp = bootyKeys["greenKeys"].Value;
                        itemsBase = new ItemsBase((int)temp);
        ...
        public static void LoadNpcs()
        {
            using (var mySqlClient = SqlDatabaseManager.GetClient())
            {
                var data = (DataTable)mySqlClient.ExecuteQueryTable("SELECT * FROM server_npcs");
                foreach (DataRow row in data.Rows)
                {
                    string name = Convert.ToString(row["name"]);
                    int shipID = Convert.ToInt32(row["shipID"]);
